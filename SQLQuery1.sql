...
SELECT departamento, 
       SUM(salario) AS salario_totalmayor8000
FROM empleados
GROUP BY departamento
...
SELECT e.nombre, e.apellido, e.salario, e.departamento
FROM empleados e
JOIN (
    SELECT departamento, AVG(salario) AS salario_promedio
    FROM empleados
...
ON e.departamento = promedio_por_departamento.departamento
WHERE e.salario > promedio_por_departamento.salario_promedio;
...
-- Calcular el número total de empleados y guardar el resultado en una variable

WITH total_empleados AS (
...
-- Combinar los resultados para calcular el porcentaje

SELECT 
    e.departamento, 
    e.num_empleados,
    (e.num_empleados * 1.0 / t.total * 100) AS porcentaje
FROM empleados_por_departamento e, total_empleados t

UNION ALL

-- Agregar el total de empleados y el 100% al final
SELECT 
    'TOTAL' AS departamento,
    t.total AS num_empleados,
    100.0 AS porcentaje
...
SELECT 
    ...
    apellido, 
    salario, 
    RANK() OVER (PARTITION BY departamento ORDER BY salario DESC) AS salario_ranking
FROM empleados;
...
SELECT 
    departamento, 
    STDEV(salario) AS desviacion_estandar_salario
FROM empleados
GROUP BY departamento;
